...
function generateAutoSifirCards(): SifirCard[] {
  ...
  // AUTO-GENERATE ONLY FOR EASY.
  // Medium and Hard must be added by admin in Supabase.
...
function isRealAdminCard(card: SifirCard) {
  ...
  // Easy questions are generated by this page.
  // Medium & Hard should only come from rows that admin really added.
  // Old auto-generated rows normally have language = auto or id starts with medium-/hard-.
...
const text = {
  bm: {
    ...
    headerTitle: "Sifir",
    back: "Balik",
    subtitle: "Easy auto 1 hingga 12. Medium dan Hard ikut soalan yang admin tambah.",
    loading: "Memuatkan kad...",
    emptyDesc: "Soalan belum tersedia. Medium dan Hard hanya akan keluar selepas admin tambah soalan.",
    cardMode: "Card Mode",
  ...
  en: {
    ...
    back: "Back",
    subtitle: "Easy auto 1 to 12. Medium and Hard use admin-added questions only.",
    loading: "Loading cards...",
    emptyDesc: "Questions are not available yet. Please wait for admin to add questions.",
    cardMode: "Card Mode",
    verticalMode: "Vertical Mode",
    wheelMode: "Spin Wheel",
    spin: "Spin Wheel",
...
function SifirDeckGame() {
  ...
  async function loadCards() {
    ...
    if (error) {
      // Fallback for older database schema that uses "level" instead of "difficulty".
      const { data: fallbackData, error: fallbackError } = await supabase
        .from("sifir_deck_questions")
        .select("id,language,question,answer,level,table_no,created_at")
        .order("created_at", { ascending: false });

      if (fallbackError) {
        console.error(fallbackError);
      } else {
        adminCards = ((fallbackData || []) as SifirCard[]).map((card) => ({
          ...card,
  ...
  function chooseSifir(num: number) {
    const filtered = allCards.filter((card) => {
      ...
      // EASY: use auto-generated normal multiplication questions only.
      if (difficulty === "easy") {
      ...
      // MEDIUM & HARD: real admin-added questions only.
      // Do not use auto-generated cards or old generated rows saved in database.

...
type FabricCanvasLike = {
  ...
  toDataURL: (options: { format: "jpeg" | "png"; multiplier: number; quality?: number }) => string;
  toJSON: () => unknown;
...
export default function WorksheetCanvas() {
  ...
  function savePDF() {
    ...
    const pageWidth = Math.round(canvasSize.width * 0.75);
    const pageHeight = Math.round(canvasSize.height * 0.75);
    ...
    function object(id: number, body: string | Uint8Array, beforeBinary = "", afterBinary = "") {
    ...
    object(
      4,
      jpegBytes,
      `<< /Type /XObject /Subtype /Image /Width ${canvasSize.width * 2} /Height ${canvasSize.height * 2} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${jpegBytes.length} >>\nstream\n`,
      "\nendstream\n"
    );
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `${exportFileName.trim() || "fd-arcadia-worksheet"}.pdf`;
    link.click();
      ...
      <header className="sticky top-0 z-20 border-b border-[var(--fd-blue)]/10 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-3">
            ...
            <button
              onClick={savePDF}
              className="inline-flex items-center gap-2 rounded-lg bg-[var(--fd-green)] px-4 py-3 text-sm font-black text-white shadow-[0_16px_38px_rgba(107,170,117,0.22)] transition hover:-translate-y-0.5"
            >
              <Download size={18} aria-hidden="true" />
              Export PDF
            </button>
